...
class CNNLSTMRegressor(nn.Module):
    """
    输入窗口长度只有8，因此卷积核不能再用 8。
    这里采用更适合 seq8 的实现：
    Conv1D(1->8, kernel=3, padding=1) -> ReLU -> MaxPool1D(2)
    -> LSTM(input=8, hidden=128) -> Linear
    """
    def __init__(self):
        super().__init__()
...
def main():
    ...
    out_dir = Path("results_cnnlstm_nasa_seq8")
    out_dir.mkdir(exist_ok=True)

    torch.save(model.state_dict(), out_dir / "cnnlstm_best.pt")
